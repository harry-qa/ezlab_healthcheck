...
- 스케줄 `17,47 * * * *`(30분 주기) + `workflow_dispatch`. 운영 이력·Pages·Slack·Issue 는 **main 정기 스케줄 실행에서만** 갱신하고, FAIL 시 GitHub Issue 자동 생성 + Slack 알림(신규·지속·복구). 설치·타입 체크·판정 규칙 테스트·리포트 생성·Pages 배포처럼 파이프라인 자체가 실패하면 별도의 "파이프라인 실패" Slack 으로 알린다(`scripts/notify_pipeline_failure.py`, 이번 런의 사이트 점검 결과도 함께 표시, 연속 실패면 첫 회와 6회(3시간)마다만). 리포트·배포가 실패한 런은 상태 이력이 앞으로 가지 않아 사이트 알림 판정이 반복되므로 사이트 알림 대신 이 알림으로 간다. 연속 장애 판정에서 같은 경로의 응답 없음(TIMEOUT)과 5xx 는 같은 장애로 본다. 보존은 실행 목록·응답시간 500회 롤링 / 월별·90일 히트맵 영구 / 리포트 폴더 120일
