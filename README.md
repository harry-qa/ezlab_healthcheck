...
이지랩 전체 서비스(이지캡쳐, 이지집, 이지파인더, 이지메모, 이지캠, 이지리더)를 대상으로 서버 생존, API 응답, UI 링크 연결, 다운로드 페이지, 콘텐츠 무결성, 이미지 로딩, 로그인 폼을 다국어(ko/en/jp) 환경에서 통합 점검합니다. 기존의 수동 점검 방식에서 벗어나 QA 공수를 절감하고 배포 품질을 확보하는 것을 목적으로 합니다.
